...
public sealed class LeaderboardScrollContent : ScrollContent<LeaderboardScrollContent, ScrollViewer<LeaderboardScrollContent>>, IScrollContent<LeaderboardScrollContent, ScrollViewer<LeaderboardScrollContent>>
{
	...
	public override void SetContent()
	{
		...
		for (int i = 0; i < StateManager.LeaderboardState.CustomLeaderboard.CustomEntries.Count; i++)
		{
			GetCustomEntry customEntry = StateManager.LeaderboardState.CustomLeaderboard.CustomEntries[i];
			LeaderboardEntry leaderboardEntry = new(Bounds.CreateNested(4, 4 + i * LeaderboardEntry.Height, 256, LeaderboardEntry.Height), customEntry);
