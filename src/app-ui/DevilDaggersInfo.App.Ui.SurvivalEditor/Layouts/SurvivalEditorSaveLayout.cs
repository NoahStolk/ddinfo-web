...
public class SurvivalEditorSaveLayout : Layout, IFileDialogLayout
{
		...
		: base(Constants.Full)
	{
		...
		TextInput fileTextInput = new(Rectangle.At(0, 64, 512, _entryHeight), false, Color.Black, Color.Gray(0.75f), Color.Gray(0.25f), Color.White, Color.White, Color.Green, Color.Gray(0.5f), 8, 2, FontSize.F8X8);
		Button saveButton = new(Rectangle.At(512, 64, 128, _entryHeight), () => SaveSpawnset(Path.Combine(_pathTextInput.Value.ToString(), fileTextInput.Value.ToString())), Color.Black, Color.White, Color.White, Color.Red, "Save", TextAlign.Left, 2, FontSize.F8X8);
	...
	public void SetComponentsFromPath(string path)
	{
		...
		foreach (string file in Directory.GetFiles(path))
			_subDirectoryButtons.Add(new Button.PathButton(new(0, ++i * _entryHeight, 1024, i * _entryHeight + _entryHeight), () => {}, Path.GetFileName(file), Color.White));
