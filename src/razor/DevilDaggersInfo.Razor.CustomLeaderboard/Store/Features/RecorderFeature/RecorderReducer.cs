...
public static class RecorderReducer
{
	...
	{
		return new(RecorderStateType.WaitingForMarker, null, state.UploadError, state.LastSuccessfulUpload);
	}

	[ReducerMethod]
	public static RecorderState ReduceFetchMarkerSuccessAction(RecorderState state, FetchMarkerSuccessAction action)
	{
		return new(RecorderStateType.Recording, action.Marker, state.UploadError, state.LastSuccessfulUpload);
	}

	[ReducerMethod]
	public static RecorderState ReduceFetchMarkerFailureAction(RecorderState state, FetchMarkerFailureAction action)
	{
		return new(RecorderStateType.WaitingForGame, null, state.UploadError, state.LastSuccessfulUpload);
	}

	[ReducerMethod]
	public static RecorderState ReduceUploadRunAction(RecorderState state, UploadRunAction action)
	{
		return new(RecorderStateType.Uploading, state.Marker, state.UploadError, state.LastSuccessfulUpload);
	}

	[ReducerMethod]
	public static RecorderState ReduceUploadRunSuccessAction(RecorderState state, UploadRunSuccessAction action)
	{
		return new(RecorderStateType.CompletedUpload, state.Marker, null, DateTime.Now);
	}

	[ReducerMethod]
	public static RecorderState ReduceUploadRunFailureAction(RecorderState state, UploadRunFailureAction action)
	{
		return new(RecorderStateType.CompletedUpload, state.Marker, action.Error, state.LastSuccessfulUpload);
	}
}
