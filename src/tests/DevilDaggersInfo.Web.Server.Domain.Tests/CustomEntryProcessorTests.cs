...
public class CustomEntryProcessorTests
{
	...
	public async Task TestHomingCount(int expected, int[] homingStored)
	{
		...
		SuccessfulUploadResponse successfulUploadSuccess = await _customEntryProcessor.ProcessUploadRequestAsync(uploadRequest);
		Assert.AreEqual(expected, successfulUploadSuccess.HomingStoredState.Value);
	}
	...
	public async Task ProcessUploadRequest_ExistingPlayer_ExistingEntry_NoHighscore()
	{
		...
		Assert.AreEqual(1, successfulUploadSuccess.SortedEntries.Count);
		Assert.IsTrue(successfulUploadSuccess.Message.StartsWith("No new highscore"));
	}
	...
	public async Task ProcessUploadRequest_ExistingPlayer_ExistingEntry_NewHighscore()
	{
		...
		_dbContext.Verify(db => db.SaveChangesAsync(default), Times.AtLeastOnce);
		Assert.AreEqual(1, successfulUploadSuccess.SortedEntries.Count);
		Assert.IsTrue(successfulUploadSuccess.Message.StartsWith("NEW HIGHSCORE"));
	}
	...
	public async Task ProcessUploadRequest_ExistingPlayer_NewEntry()
	{
		...
		_dbContext.Verify(db => db.SaveChangesAsync(default), Times.AtLeastOnce);
		Assert.IsTrue(successfulUploadSuccess.Message.StartsWith("Welcome"));
	}
