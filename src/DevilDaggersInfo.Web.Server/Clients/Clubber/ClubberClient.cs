...
public class ClubberClient
{
	...
	public async Task<List<DdUser>?> GetUsers()
	{
		try
		{
			return await _httpClient.GetFromJsonAsync<List<DdUser>>("https://clubber.up.railway.app/users") ?? throw new InvalidOperationException("Response was null which was not expected.");
		}
		catch (Exception ex)
		{
