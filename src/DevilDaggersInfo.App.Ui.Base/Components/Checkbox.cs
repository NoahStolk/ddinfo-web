...
public class Checkbox : AbstractCheckbox
{
	...
	public override void Render(Vector2i<int> parentPosition)
	{
		...
		Root.Game.UiRenderer.RenderCenter(scale, parentPosition + center, Depth + 1, Hover ? new(0.25f) : Vector3.Zero);

		if (CurrentValue)
			Root.Game.UiRenderer.RenderCenter(scale - borderTickVec, parentPosition + center, Depth + 2, new(0.75f));
	}
}
