...
public static class MarkupUtils
{
	...
	{
		return new MarkupString($"""<span class="font-goethe text-lg {dagger.Name.ToLower()}">{dagger.Name} Dagger</span>""");
	}
	...
	public static MarkupString CustomLeaderboardDeathString(byte? deathType, GameVersion gameVersion = GameConstants.CurrentVersion, string textSizeClass = "text-lg")
	{
		if (!deathType.HasValue)
			return new MarkupString($"""<span style="color: {MarkupStrings.NoDataColor};" class="font-goethe {textSizeClass}">N/A</span>""");
	...
	public static MarkupString DeathString(Death? death, string textSizeClass = "text-lg")
	{
		...
		return new MarkupString($"""<span style="{style}" class="font-goethe {textSizeClass}">{death?.Name ?? "Unknown"}</span>""");
	}

	public static MarkupString UpgradeString(Upgrade upgrade)
	{
		string style = $"color: {upgrade.Color.HexCode};";
		return new MarkupString($"""<span style="{style}" class="font-goethe text-lg">{upgrade.Name}</span>""");
	}
	...
	public static MarkupString EnemyString(Color enemyColor, string enemyName, bool plural = false)
	{
		...
		return new MarkupString($"""<span style="{style}" class="font-goethe text-lg">{enemyName}{(plural ? "s" : string.Empty)}</span>""");
	}

	public static MarkupString LeaderboardTime(double timeInSeconds)
	{
		return new MarkupString($"""<span class="{Daggers.GetDaggerFromSeconds(GameConstants.CurrentVersion, timeInSeconds).Name.ToLower()} font-goethe text-lg">{timeInSeconds.ToString(StringFormats.TimeFormat)}</span>""");
	}

	public static MarkupString ProhibitedString(bool isProhibited)
	{
		if (isProhibited)
			return new MarkupString("<span class='text-orange'>Prohibited</span>");

		return new MarkupString("<span class='text-green'>OK</span>");
	}
}
