...
namespace DevilDaggersWebsite.Pages.Admin.AdminTests
{
	public class MigrateSpawnsetsToDatabaseModel : PageModel
	{
		...
		public void OnGet()
		{
			Dictionary<string, dynamic> dict = JsonConvert.DeserializeObject<Dictionary<string, dynamic>>(Io.File.ReadAllText(Path.Combine(env.WebRootPath, "spawnsets", "Settings", "Settings.json")));

			List<SpawnsetFile> spawnsetFiles = new List<SpawnsetFile>();
			int i = 1;
			foreach (KeyValuePair<string, dynamic> kvp in dict.OrderBy(kvp => kvp.Value.LastUpdated))
			{
				string name = kvp.Key.Substring(0, kvp.Key.LastIndexOf('_'));
				string author = kvp.Key.Substring(kvp.Key.LastIndexOf('_') + 1);

				spawnsetFiles.Add(new SpawnsetFile
				{
					Id = i++,
					Name = name,
