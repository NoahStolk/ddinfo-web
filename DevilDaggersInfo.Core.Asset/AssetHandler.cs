...
public sealed class AssetHandler
{
	...
	private AssetHandler()
	{
		...
		static Stream GetContentStream(string relativeContentName)
			=> Assembly.GetExecutingAssembly().GetManifestResourceStream($"DevilDaggersInfo.Core.Asset.{relativeContentName}") ?? throw new($"Could not retrieve content stream '{relativeContentName}'.");
	}
