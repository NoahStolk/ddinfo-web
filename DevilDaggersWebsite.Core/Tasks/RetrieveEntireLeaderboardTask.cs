...
namespace DevilDaggersWebsite.Core.Tasks
{
	public class RetrieveEntireLeaderboardTask : AbstractTask
	{
		...
		{
			Leaderboard = await DdHasmodaiClient.GetScores(1);

			for (int i = 1; i < MaxPages; i++)
			{
				Leaderboard nextLeaderboard = await DdHasmodaiClient.GetScores(i * 100 + 1);
		...
		{
			Leaderboard leaderboard = await DdHasmodaiClient.GetScores(1);
